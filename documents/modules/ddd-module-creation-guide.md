...
Define the "What". No Laravel dependencies allowed here (except basics). **Always use `declare(strict_types=1);`.**
...
1. **Commands/Queries:** Create simple DTOs (`CreateTagCommand`, `ListTagsQuery`).
    * *Note:* Default `TagDTO` does NOT include `created_at`/`updated_at` by default in generation to simplify initial setup, but can be added if needed.
...
2. **Read Models:** `EloquentTagReader` (and `CachingTagReader` decorator).
    ...
    * *Config Injection:* Pass config values (TTL, Prefix) via constructor in ServiceProvider. Do NOT use `config()` helper inside the class.
3. **Service Provider:** Bind everything in `TagServiceProvider`.
    * **Crucial:** Explicitly bind `TagCacheInvalidator` (and other listeners/readers requiring config) in `register()` to inject configuration values from `config/tag.php`.
...
3. **Controller:** `TagController`.
    ...
    * *Logic:* Use `Intent` Enum for branching logic (e.g., `CreateAndContinue`). Use strict comparison (`===`).
...
* **Inject Config:** Listeners and Readers should receive config values via constructor, injected by the ServiceProvider.
...
* ❌ **Don't** use `config()` helper inside Domain or Infrastructure classes. Inject values.
...
5. **Strict Types:** All generated PHP files must have `declare(strict_types=1);`.
6. **Config Injection:** ServiceProvider must explicitly bind classes that need config values (like CacheInvalidator).
